...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      v2Entities: {
        masters: [
          { id: 'master_gemini', name: 'Gemini 1.5 Pro (Architect)', role: 'Master Principal (Architect)', status: process.env.GEMINI_API_KEY ? 'ONLINE' : 'OFFLINE', power: 100, reason: process.env.GEMINI_API_KEY ? 'Connected' : 'Not configured' },
          { id: 'master_fallback', name: 'GPT-4o (Oracle)', role: 'Oracle (Sentiment)', status: process.env.OPENAI_API_KEY ? 'ONLINE' : 'OFFLINE', power: 80, reason: process.env.OPENAI_API_KEY ? 'Connected' : 'Not configured' },
