 ...
 *
 * SCOPE: Discovery phase, LOG-ONLY. No DB writes, no signal injection into Moltbook/Scout.
 * User explicitly approved violating §4 TRADE AI Zero-features on 2026-04-19 for this probe.
 *
 ...
 *
 * KILL-SWITCHES (both in env):
 *   POLYMARKET_INGEST_ENABLED=0  → 503, endpoint dark
 ...
 *   (2) LOG-ONLY = NO side effects on TRADE AI state. Promoting to write-mode requires
 *       explicit review + new endpoint, never edit this one in-place.
 ...
 *   1. gcloud run services update trade-ai --update-env-vars POLYMARKET_INGEST_ENABLED=0
...
export async function POST(req: NextRequest) {
  ...
  // 6. LOG ONLY — NO DB writes. Truncated sample to avoid log spam.
  ...
  log.info('ingest', { count, sample: sampleStr });
