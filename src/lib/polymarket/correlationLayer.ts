 ...
 *   (2) Moltbook karma e accesibil prin `sentimentCoupling` existent (stub
 *       pentru acum: wire-up complet in FAZA 3.6 cand Moltbook are API stabil).
 ...
 *   POLYMARKET_GOLDSKY_CORRELATION=0 → gofdskyConfirm=1.0 (celelalte active)
 */
...
// ── Moltbook karma stub ───────────────────────────────────────
// Hook-point: wire in Moltbook karma signal in FAZA 3.6. For now returns 1.0
// unless env override POLY_KARMA_OVERRIDE is set (for test).
function computeMoltbookKarma(_market: PolyMarket): { value: number; note: string } {
  ...
  return { value: 1.0, note: 'stub (FAZA 3.6 wire-up pending)' };
}
...
export async function correlateDecision(
  ...
  const karma = computeMoltbookKarma(market);
